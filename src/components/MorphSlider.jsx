...
class MorphEngine {
  constructor(container, { items, startIndex, reducedMotion, getOptions, onIndexChange, dprCap }) {
    ...
    // Belt-and-suspenders re-measures: the very first `resize()` above can
    // land a moment before the surrounding layout has actually settled
    // (webfonts swapping in above this frame and nudging its height, the
    // mobile URL bar collapsing right after load, this component mounting
    // underneath the still-animating loading overlay, etc.). Any of those
    // shifts the container's real box a hair after we already measured it,
    // and — unlike the OVERSCAN below — nothing here guarantees the
    // ResizeObserver's follow-up callback fires before the first frames get
    // painted. Re-running resize() a few more times over the next second,
    // plus once more when fonts/window load actually finish, catches those
    // cases without waiting on the user to toggle fullscreen to force a
    // fresh measurement.
    this.boundResize = () => this.resize();
    requestAnimationFrame(() => requestAnimationFrame(this.boundResize));
    this.settleTimeouts = [120, 350, 900].map(ms => setTimeout(this.boundResize, ms));
    if (document.readyState !== 'complete') {
      window.addEventListener('load', this.boundResize, { once: true });
    }
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(this.boundResize).catch(() => {});
    }
  ...
  resize() {
    ...
    // A few extra CSS px are rendered beyond the container's measured (often
    // fractional) size and then clipped by the frame's own `overflow:
    // hidden`. Without this, the canvas's real edge and the container's true
    // edge can land a hair apart — usually only right after mount, before
    // fonts/layout fully settle — leaving a thin sliver of the frame's own
    // background showing along the right/bottom edge until something (e.g.
    // toggling fullscreen) forces a fresh, accurate resize. Overshooting on
    // every resize is cheap and makes that gap impossible regardless of what
    // caused it. (Bumped from 2 to 6: 2px wasn't always enough to survive
    // the layout settling this component can go through right after mount —
    // see the extra re-measure passes in the constructor.)
    const OVERSCAN = 6;
    const w = Math.max(rect.width, 1) + OVERSCAN;
    const h = Math.max(rect.height, 1) + OVERSCAN;
  ...
  destroy() {
    ...
    if (this.settleTimeouts) this.settleTimeouts.forEach(id => clearTimeout(id));
    if (this.boundResize) window.removeEventListener('load', this.boundResize);
...
const MorphSlider = forwardRef(function MorphSlider(
  ...
  // Optional imperative handle: lets a parent trigger next()/prev() itself
  // (e.g. to drive its own autoplay loop) without fighting MorphSlider's own
  // built-in autoplay/hover logic. Purely additive — nothing here changes
  // how the component behaves when no ref is passed.
  ...
  // Lets a parent track which slide is showing (e.g. to render its own
  // caption UI) without that parent's callback identity — likely to change
  // on every render — ever forcing this effect to tear down and rebuild the
  // whole WebGL engine (same fix as CircularGallery's onSelect/onActiveIndexChange).
  ...
  useEffect(() => {
    ...
    setIndex(startIndex);
    if (onIndexChangeRef.current) onIndexChangeRef.current(startIndex);

    return () => {
      engine.destroy();
      engineRef.current = null;
    };
    // Deliberately excludes onIndexChange — see comment above.
